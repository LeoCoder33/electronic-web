...
def pf_optimal(x, P_pv, Z, S_load, Y):
    ...
    tmp = (tmp - x.T.dot(P_pv) - 1j * 10 * x.T.dot(P_pv) * np.tan(np.arccos(0.9))) / (
                Sb * 1000)  # 计算加入分布式电源后系统容量的标幺值，在这里直接用功率因数来折算无功
    Z[:, 3] = tmp[:, 0]
    ZZ = Z[:, 3]
    ...
    for k in range(N_bus, 0, -1):  # 后推线路容量
        A = np.where(Z[:, 0] == k)[0]
        if len(A) == 0:  # 找到末节点号
            temp = Z[k - 1, 3] + (np.real(Z[k - 1, 3]) ** 2 + np.imag(Z[k - 1, 3]) ** 2) * Z[k - 1, 2] / (
                        U[k - 1, 0] ** 2)
            s[k - 1, 0] = temp
        else:
